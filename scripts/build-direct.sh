...
build_root="${SPACELENS_DIRECT_BUILD_ROOT:-/private/tmp/macossoftware-spacelens-direct}"
derived_data="$build_root/DerivedData"
source_dir="$build_root/src"
stage_dir="$build_root/stage"
output_dir="${SPACELENS_DIRECT_OUTPUT_DIR:-/Users/$USER/Desktop/MacOSSoftware Direct Releases/SpaceLens}"
...
[[ "$build_root" == /private/tmp/macossoftware-spacelens-direct* || -n "${SPACELENS_DIRECT_BUILD_ROOT:-}" ]] || {
...
app_path="$derived_data/Build/Products/Direct/SpaceLens.app"
[[ -d "$app_path" ]] || { echo "SpaceLens.app was not produced" >&2; exit 1; }
bundle_id="$(defaults read "$app_path/Contents/Info" CFBundleIdentifier)"
[[ "$bundle_id" == "com.lukemclaughlin.spacelens.direct" ]] || {
  echo "Unexpected direct bundle identifier: $bundle_id" >&2
  exit 1
}

executable="$app_path/Contents/MacOS/SpaceLens"
if otool -L "$executable" | grep -q 'StoreKit.framework'; then
  echo "Refusing to package a website build linked to StoreKit" >&2
  exit 1
fi
if rg -a -q 'Start Free Trial|Restore Purchases|Manage Subscription|Auto-renews|Subscribe Annually|Try Premium' "$executable"; then
  echo "Refusing to package a website build containing subscription purchase UI" >&2
  exit 1
fi
...
dmg_path="$output_dir/SpaceLens-${version}-direct.dmg"
rm -f "$dmg_path"
hdiutil create -volname "SpaceLens Direct" -srcfolder "$stage_dir" -ov -format UDZO "$dmg_path"
